 ...
 */
export function exportSessionLogMarkdown(
  log: SessionLogData,
  projectName: string
): void {
  const lines: string[] = [];
  ...
  // Header
  lines.push(`# ${log.metadata.projectName}`);
  lines.push(`**Critical Code Studies Session Log**`);
  lines.push("");
  lines.push(`*CCS-WB v${log.metadata.appVersion} | CCS Methodology v${log.metadata.ccsSkillVersion}*`);
  lines.push("");
  ...
  log.conversationLog.forEach((msg) => {
    ...
    lines.push(`### ${role}${phase}${favourite}`);
  ...
  lines.push("---");
  lines.push("");
  lines.push("*Generated by CCS Workbench*");
